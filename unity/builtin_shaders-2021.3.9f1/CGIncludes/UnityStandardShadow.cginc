...
// and inputting VPOS in the pixel shader, since they both map to "POSITION" semantic on some platforms, and then things don't go well.
// 我们必须在顶点着色器中单独输出 SV_POSITION，并在像素着色器中输入 VPOS、 并在像素着色器中输入 VPOS，因为在某些平台上，它们都映射为 "POSITION "语义，这样就会出现问题
