...
#    define UNITY_READ_SHADOW_COORDS(input) READ_SHADOW_COORDS(input)
...
#elif defined(SHADOWS_SCREEN) && !defined(LIGHTMAP_ON) && !defined(UNITY_NO_SCREENSPACE_SHADOWS) // no lightmap uv thus store screenPos instead 没有光贴图 uv，因此只能存储 screenPos
    ...
    // 如果我们有两个方向的灯光，就会出现这种情况。主灯光在 GI 代码中处理，但第二个方向的灯光可以有阴影屏幕和遮罩。
    // - Disabled on ES2 because WebGL 1.0 seems to have junk in .w (even though it shouldn't)
    // 在 ES2 上禁用，因为 WebGL 1.0 的 .w 中似乎有垃圾（尽管不应该有垃圾）。
...
// 用于计算光照衰减系数，参数一 为返回值（光照衰减系数），参数二 用于阴影计算，参数三 是世界坐标
#ifdef DIRECTIONAL
