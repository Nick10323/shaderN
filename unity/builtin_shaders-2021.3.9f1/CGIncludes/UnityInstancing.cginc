...
// 应在顶点着色器/片段着色器的最开始使用，以便后续代码可以访问全局 unity_InstanceID。同时，程序函数也会被调用来设置实例数据。
// - UNITY_TRANSFER_INSTANCE_ID     Copy instance ID from input struct to output struct. Used in vertex shader.   将实例 ID 从输入结构复制到输出结构。在顶点着色器中使用。
    ...
    void UnitySetupInstanceID(uint inputInstanceID)
    {
                ...
                // because otherwise,  the unity shader compiler will emit a bitfieldInsert function. bitfieldInsert requires support for glsl version 400 or later.
                // Therefore the generated glsl code will fail to compile on lower end devices. By changing the way we calculate the stereo eye index,
                // we can help the shader compiler to avoid emitting the bitfieldInsert function and thereby increase the number of devices we can run stereo instancing on.
                // 对于 GLES3，我们必须以不同的方式计算立体眼球索引，否则 unity 着色器编译器将发出 bitfieldInsert 函数。
                // 因此，生成的 glsl 代码将无法在低端设备上编译。 通过改变立体眼球索引的计算方式，我们可以帮助着色器编译器避免触发 bitfieldInsert 函数，从而增加可运行立体实例化的设备数量。
                unity_StereoEyeIndex = round(fmod(inputInstanceID, 2.0));
                unity_InstanceID = unity_BaseInstanceID + (inputInstanceID >> 1);
            #else
                // stereo eye index is automatically figured out from the instance ID   根据实例 ID 自动计算出立体眼索引
    ...
    void UnitySetupCompoundMatrices();
        ...
        #else
            void UNITY_INSTANCING_PROCEDURAL_FUNC();   // forward declaration of the procedural function 程序函数的正向声明
