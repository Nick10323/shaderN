...
// 注意：迪斯尼漫反射必须乘以 diffuseAlbedo / PI。这是在本函数之外完成的。
...
// purposes, mostly for DX9 SM2.0 level. Most of the math is being done on these (1-x) values, and that saves a few precious ALU slots.
// 注：BRDF 输入点使用平滑度和 oneMinusReflectivity（负反射率）进行优化
// 主要用于 DX9 SM2.0 级别。大部分数学运算都是在这些 (1-x) 值上完成的，这样可以节省一些宝贵的 ALU 插槽。


// Main Physically Based BRDF   主要物理 BRDF
// Derived from Disney work and based on Torrance-Sparrow micro-facet model   源自迪斯尼作品，基于Torrance-Sparrow micro-facet模型
//
...
half4 BRDF1_Unity_PBS (half3 diffColor, half3 specColor, half oneMinusReflectivity, half smoothness, float3 normal, float3 viewDir,
...
// 对于可见像素来说，NdotV 不应该是负值，但由于透视投影和法线映射，可能会出现这种情况。
// In this case normal should be modified to become valid (i.e facing camera) and not cause weird artifacts.
// 在这种情况下，应修改法线使其有效（即朝向摄像机），而不会导致奇怪的伪影。
// but this operation adds few ALU and users may not want it. Alternative is to simply take the abs of NdotV (less correct but works too).
// 但这种操作会增加一些 ALU，用户可能不希望这样做。另一种方法是简单地取 NdotV 的哂值（虽然不那么正确，但也有效）。
// Following define allow to control this. Set it to 0 if ALU is critical on your platform.
// 下面的定义允许对其进行控制。如果 ALU 在您的平台上非常重要，请将其设置为 0。
// This correction is interesting for GGX with SmithJoint visibility function because artifacts are more visible in this case due to highlight edge of rough surface
// 对于使用 SmithJoint 可见度函数的 GGX 来说，这种修正非常有趣，因为在这种情况下，由于粗糙表面的高亮边缘，伪影更加明显。
// Edit: Disable this code by default for now as it is not compatible with two sided lighting used in SpeedTree.
// 编辑：默认情况下暂时禁用此代码，因为它与 SpeedTree 中使用的双面光照不兼容。
#define UNITY_HANDLE_CORRECTLY_NEGATIVE_NDOTV 0

#if UNITY_HANDLE_CORRECTLY_NEGATIVE_NDOTV
    // The amount we shift the normal toward the view vector is defined by the dot product.   法线向视图向量的移动量由点积决定。
    half shiftAmount = dot(normal, viewDir);
    normal = shiftAmount < 0.0f ? normal + viewDir * (-shiftAmount + 1e-5f) : normal;
    // A re-normalization should be applied here but as the shift is small we don't do it to save ALU. 这里应该重新规范化，但由于移位很小，为了节省 ALU，我们没有这样做。
