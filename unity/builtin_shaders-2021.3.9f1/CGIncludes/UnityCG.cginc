...
// 用于 ForwardBase 通道： 计算 4 个点光源的漫反射照明，数据以特殊方式打包。
...
fixed3 UnpackNormalWithScale(fixed4 packednormal, float scale)
{
        ...
        // Unpack normal as DXT5nm (1, y, 1, x) or BC5 (x, y, 0, 1)
        // Note neutral texture like "bump" is (0, 0, 1, 1) to work with both plain RGB normal and DXT5nm/BC5
        // 注意中性纹理（如 "凹凸"）的值为 (0,0,1,1)，以便与普通 RGB 法线和 DXT5nm/BC5 配合使用。
        packednormal.x *= packednormal.w;
...
// Z buffer to linear 0..1 depth Z 缓冲区至线性 0...1 深度
inline float Linear01Depth( float z )
{
    return 1.0 / (_ZBufferParams.x * z + _ZBufferParams.y);
}

// Z buffer to linear depth Z 缓冲区至线性深度
...
float4 UnityClipSpaceShadowCasterPos(float4 vertex, float3 normal)
{
    ...
    if (unity_LightShadowBias.z != 0.0)
    {
        ...
        // apply normal offset bias (inset position along the normal) bias needs to be scaled by sine between normal and light direction
        // 应用法线偏移偏置（沿法线嵌入位置），偏置需要按法线和光照方向之间的正弦比例缩放
        // (http://the-witness.net/news/2013/09/shadow-mapping-summary-part-1/)
        // unity_LightShadowBias.z contains user-specified normal offset amount scaled by world space texel size.
        // unity_LightShadowBias.z 包含用户指定的法线偏移量，该偏移量按世界空间像素大小缩放。
...
float4 UnityApplyLinearShadowBias(float4 clipPos)
{
    ...
    // 对于支持深度立方体贴图的点光源，偏置会在阴影贴图采样的片段着色器中应用。
    // This is because the legacy behaviour for point light shadow map cannot be implemented by offseting the vertex position in the vertex shader generating the shadow map.
    // 这是因为在生成阴影贴图的顶点着色器中，无法通过偏移顶点位置来实现点光源阴影贴图的传统行为。
#if !(defined(SHADOWS_CUBE) && defined(SHADOWS_CUBE_IN_DEPTH_TEX))
    #if defined(UNITY_REVERSED_Z)
        // We use max/min instead of clamp to ensure proper handling of the rare case where both numerator and denominator are zero and the fraction becomes NaN.
        // 我们使用最大/最小值而不是钳位，以确保正确处理分子和分母都为零，分数变成 NaN 的罕见情况。
